...
app_new.set('port', (process.env.PORT || 3001));

app_new.get('*', (req, res) => {
    res.send('Madden Companion Exporter');
});

app_new.post('/:username/:platform/:leagueId/leagueteams', (req, res) => {
...
app_new.post('/:username/:platform/:leagueId/standings', (req, res) => {
    const db = admin.database();
    const ref = db.ref();
    let body = '';
    req.on('data', chunk => {
...
// ROSTERS
app_new.post('/:username/:platform/:leagueId/freeagents/roster', (req, res) => {
    const db = admin.database();
    const ref = db.ref();
    const {
...
app_new.post('/:username/:platform/:leagueId/team/:teamId/roster', (req, res) => {
    const db = admin.database();
    const ref = db.ref();
    const {
        params: { username, leagueId, teamId }
...
app_new.listen(app_new.get('port'), () =>
    console.log('Madden Data is running on port', app_new.get('port'))
);
